...
Usage: d9.py -n [database name] -d [database host] -u [database username] -p [password]
-f [filename]
"""
...
class Annovar:
    ...
    def insert_table(self, table, variant):
        ...
        c_names = ', '.join(dcolumns)
        # Format the VALUES as '(%(name)s, %(chr_id)s)'
        values = ', '.join(['%({})s'.format(k) for k in dcolumns])
        query = f"INSERT INTO {table} ({c_names}) VALUES ({values})"
